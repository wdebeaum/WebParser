...
;; Adjust this to allow dump to find relocated WordNet files
(setf wordfinder::*wordnet-basepath* "/u/www/research/trips/parser/etc/WordFinder/wordnet/dict/")
...
(defun run-cgi ()
  (let* ((*package* (find-package :parser))
    ...
    ;; (p input)
    ;; No, parse without output and then show output of LF
    (parse '(start))
    (parse `(word ,input :frame (0 ,(length input))))
    (parse '(return))
    (lf)
    (format t "~%;;;;; parse tree:~%~S~%" (show-tree))
