...
function loadONTLI(onttype) {
  ...
  xmlrequest.open("GET", "lex-ont?side=ont&ret=xml&q=" + onttype, false)
...
function setTargets() {
  if (top.location.href != window.location.href) { // we're in frames
    ...
    for (var i = 0; i < links.length; i++) {
      if (/lex-ont\?side=lex/.test(links[i].href)) {
...
function filterByRoles() {
  ...
  if (roles.length > 0) {
    ...
    var xmlrequest = new XMLHttpRequest()
    xmlrequest.open("GET", "lex-ont?side=ont&ret=types-for-roles&q=" + roles.join(','), false)
    xmlrequest.send(null)
